        ...
        with ui.card().classes("col-start-2"):
            ...
            with ui.row():
                ui.button("A", on_click=lambda: ui.navigate.to(
                    f"/group/a/{upper_bound}/{lower_bound}")).classes("bg-red text-black text-lg")
                ui.button("B", on_click=lambda: ui.navigate.to(
                    f"/group/b/{upper_bound}/{lower_bound}")).classes("bg-green text-black text-lg")
                ui.button("C", on_click=lambda: ui.navigate.to(
                    f"/group/c/{upper_bound}/{lower_bound}")).classes("bg-yellow text-black text-lg")
            ...
            with ui.row():
                ...
                ui.button(text="確認修改",
                          on_click=lambda: set_new_bound(
                              upper_num.value, lower_num.value)
                          ).classes("text-lg")
